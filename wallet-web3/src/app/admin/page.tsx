...
export default function AdminPage() {
    ...
    try {
      ...
      setStatus(
        response.ok
          ...
          : {
              kind: "error",
              text: body.error || "No se pudo iniciar la migración.",
            },
      );
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError(
        error instanceof Error ? error.message : "No se pudo validar el token.",
      );
    } finally {
  ...
  const currentCategory = useMemo(
    () =>
      data?.categories.find((category) => category.name === activeCategory) ||
      null,
    [data, activeCategory],
  );
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div>
            ...
            <div className="text-sm text-white/60 mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
              <span>
                Traits in{" "}
                <span className="font-mono text-white/80">{globalDirName}</span>{" "}
        ...
        {storage && !storage.healthy && (
          <div className="rounded-xl border-2 border-red-500 bg-red-500/20 px-6 py-5">
            ...
            <p className="text-red-200/90 mt-2">
              La carpeta de traits del servidor no es un volumen persistente.
              Todo lo que subas, renombres o borres va a desaparecer la próxima
              vez que el servidor se reinicie, sin ningún aviso.
            </p>
            ...
            {storage.relativePathMistake && (
              <p className="text-sm text-red-200 mt-1">
                Esa ruta es <strong>relativa</strong>. El mount path de un
                volumen es siempre absoluto, como{" "}
                <span className="font-mono">/data/traits</span>.
              </p>
            )}
            <p className="text-sm text-red-200/70 mt-1">
              En Railway, el mount path del volumen tiene que ser exactamente
              esa ruta.
              {storage.marker &&
        ...
        {assets && (!assets.readyForDns || assets.seeding.running) && (
          <div className="rounded-xl border border-amber-500/50 bg-amber-500/10 px-6 py-5">
            ...
            <p className="text-sm text-amber-100/80 mt-1">
              ...
              <span className="font-mono">{assets.originUrl}</span> a este
              servidor. Tiene que estar completa <strong>antes</strong> de
              apuntar el dominio acá: si no, la colección entera se ve rota en
            ...
            {assets.seeding.running ? (
              <div className="mt-4">
                ...
                <p className="mt-2 text-sm text-amber-100/70">
                  {assets.seeding.done} de {assets.seeding.total} tokens. Podés
                  cerrar esta página: sigue corriendo en el servidor.
                </p>
              </div>
            ) : (
              <button
        ...
        {publisher && !publisher.ok && (
          <div className="rounded-xl border-2 border-red-500 bg-red-500/20 px-6 py-5">
            ...
            <p className="text-sm text-red-200 mt-1">
              Los holders pueden guardar, pero el cambio no se va a ver en
              ninguna wallet.
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            {data?.categories.map((category) => {
              const total = category.directories.reduce(
                (sum, directory) => sum + directory.traits.length,
                0,
              );
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/5 border border-white/10 rounded-xl p-6">
              ...
              {loading ? (
                ...
                <p className="text-blue-200 py-16 text-center">
